...
async function getmyInfoV2(connection, userId) {
  const Query = `
    SELECT U.userId, nickName,
            ...
            when job = 'ACC' then '재무/회계'
            when job = 'CUS' then 'CS'
            end as job, profileImageUrl, pushOn, nameChanged,
            case when DATEDIFF(DATE_FORMAT(now(),'%Y-%m-%d'), jobChanged) > 90
            then 'Y'
            else 'N' end as jobChangePossible, pace
            FROM User U
            WHERE U.userId = ?;
      `;
  const [Rows] = await connection.query(Query, userId);
...
// 타인의 마이페이지 정보
async function getUserInfoV2(connection, userId) {
  const Query = `
    SELECT U.userId, nickName,
            case when gender='M' then '남성'
              else case when gender='F' then '여성'
            ...
            when job = 'ACC' then '재무/회계'
            when job = 'CUS' then 'CS'
            end as job, profileImageUrl, pace
            FROM User U
            WHERE U.userId = ?;
      `;
  const [Rows] = await connection.query(Query, userId);
...
async function getBM2(connection, userId) {
  ...
  case when runnerGender='F' then '여성'
  end end end as gender, whetherEnd, B.userId, J.job, peopleNum, contents, P.pace, P.afterParty,
  EXISTS (SELECT bookmarkId FROM Bookmarks
...
async function getMyPosting2(connection, userId) {
  ...
  inner join User U on RP.userId = U.userId
  group by postId) J on J.postId = P.postId
  LEFT JOIN RunningLog RL ON RL.userId = P.postUserId AND RL.gatheringId = R.gatheringId AND RL.status != 'D'
  WHERE postUserId = ?;
  `;
...
async function getUserRunning2(connection, userId) {
  ...
  INNER JOIN User U on U.userId = P.postUserId
  INNER JOIN Running R on R.postId = P.postId
  INNER JOIN (SELECT * FROM RunningPeople WHERE userId = ? AND whetherAccept = 'Y') RPP on R.gatheringId = RPP.gatheringId
...
// 유저 러닝 페이스 정보 업데이트
async function addUserPace(connection, pace, userId) {
  const Query = `update User set pace = ? where userId = ?;`;
  const [Rows] = await connection.query(Query, [pace, userId]);
  return Rows;
}
